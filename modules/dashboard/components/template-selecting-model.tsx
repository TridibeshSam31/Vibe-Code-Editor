...
// Implement Filter Here means we have to filter out the template on searching the template by name 
const filteredTemplates = templates.filter((template)=>{
 ...
 //function for selecting the template so that when the user clicks on the template say react it will show is kind of highlighted
  const handleSelectTemplate = (templateId: string) => {
    setSelectedTemplate(templateId);
  };
 //after seleting the template on which u have to work then the next step is to configure it to work on it
  ...
  const handleCreateProject = () => {
    if (selectedTemplate) {
      ...
      onSubmit({
        ...
        template: templateMap[selectedTemplate]||"REACT", //templateMap mai hum selectedTemplate ko pass kr denge
                    ...
                    filteredTemplates.map((template) => (
                      <div
                        ...
                        className={`relative flex p-6 border rounded-lg cursor-pointer transition-all duration-300 hover:scale-[1.02] 
                            ...
                            `}
                        onClick={() => handleSelectTemplate(template.id)} //what we did here is added a special object type feature to the css that will trigger only when we select the template then it will change the border color 
                      >
